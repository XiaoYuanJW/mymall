...
public class ForeReviewController extends BaseController {
    ...
    public String goToPage(HttpSession session, Map<String, Object> map,
        ...
        logger.info("获取订单项信息");
        OrderItem orderItem = orderItemService.get(orderItem_id);
        if (orderItem == null) {
            logger.warn("订单项不存在，返回订单页");
            return "redirect:/order/0/10";
        }
        if (!orderItem.getOrderItem_user().getUser_id().equals(userId)) {
        ...
        Order order = orderService.get(orderItem.getOrderItem_order().getOrder_id());
        if (order == null || order.getOrder_status() != 3) {
            logger.warn("订单项状态有误，返回订单页");
            return "redirect:/order/0/10";
        }
    ...
    public String addReview(HttpSession session, Map<String, Object> map,
        ...
        Review review = new Review()
                .setReview_product(orderItem.getOrderItem_product())
        ...
        return "redirect:/product/" + orderItem.getOrderItem_product().getProduct_id();
    }
