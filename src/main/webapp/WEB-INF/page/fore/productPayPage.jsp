...
<div class="content">
    <div class="order_div">
        <c:choose>
            <c:when test="${fn:length(requestScope.order.orderItemList)==1}">
                <div class="order_name">
                    <span>校园超市 -- ${requestScope.order.orderItemList[0].orderItem_product.product_name}</span>
                </div>
                <div class="order_shop_name">
                    <span>卖家昵称：校园${requestScope.order.orderItemList[0].orderItem_product.product_category.category_name}旗舰店</span>
                </div>
            </c:when>
            <c:otherwise>
                <div class="order_name">
                    <span>校园超市 -- 合并订单：${fn:length(requestScope.order.orderItemList)}笔</span>
                </div>
            </c:otherwise>
        </c:choose>
        <div class="order_price">
            ...
            元
            <span class="price_unit">（模拟付款）</span>
        </div>
    </div>
    <div class="order_pay_div">
        <script>
            function pay() {
                $.ajax({
                    url: "${pageContext.request.contextPath}/order/pay/${requestScope.order.order_code}",
                    type: "PUT",
                    data: null,
                    dataType: "json",
